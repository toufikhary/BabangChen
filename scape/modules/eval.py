...
#
""" scape module for executing code and terminal commands from Telegram. """
import asyncio
...
async def evaluate(query):
    ...
    if expression in ("scape.session", "config.env"):
...
execute. Use .help exec for an example.```"
    ...
    if code in ("scape.session", "config.env"):
...
async def terminal_runner(term):
    ...
    if command in ("scape.session", "config.env"):
