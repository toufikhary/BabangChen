...
#
""" scape module for getting information about the server. """
...
import psutil
from scape import CMD_HELP, ALIVE_NAME, BOT_VER, ALIVE_LOGO, bot, StartTime
...
async def bot_ver(event):
    ...
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@",
        ...
        if which("git") is not None:
            ...
            await event.edit("`scape Version: "
                             f"{verout}"
                             "` \n"
                             "`Revision: "
                             f"{revout}"
