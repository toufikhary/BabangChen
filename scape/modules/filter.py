...
#
""" scape module for filter commands """
...
async def filter_incoming_handler(handler):
    ...
    try:
        if not (await handler.get_sender()).bot:
            try:
                from scape.modules.sql_helper.filter_sql import get_filters
...
async def add_new_filter(new_handler):
    ...
    try:
        from scape.modules.sql_helper.filter_sql import add_filter
    except AttributeError:
...
async def remove_a_filter(r_handler):
    ...
    try:
        from scape.modules.sql_helper.filter_sql import remove_filter
    except AttributeError:
...
CMD_HELP.update({
    "filter":
    "`.filters`\
    \nUsage: Lists all active scape filters in a chat.\
