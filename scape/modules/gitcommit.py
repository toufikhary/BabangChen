...
# from scape.events import humanbytes, progress, time_formatter
from scape import CMD_HELP, GITHUB_ACCESS_TOKEN, GIT_REPO_NAME, bot


GIT_TEMP_DIR = "./scape/temp/"
...
async def download(event):
    ...
    if GIT_REPO_NAME is None:
        await event.edit("`Please ADD Proper Github Repo Name of your scape`")
        return 
...
async def git_commit(file_name,mone):        
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="'+file_name+'")':
            ...
            create_file = False
    file_name = "scape/modules/" + file_name		
    if create_file is True:
        file_name = file_name.replace("./scape/temp/","")
        print(file_name)
        try:
            ...
            ccess = GIT_REPO_NAME
            ccess = ccess.strip()
            await mone.edit(f"`Commited On Your Github Repo`\n\n[Your Modules](https://github.com/{ccess}/tree/sql-extended/scape/modules/)")
        except:    
