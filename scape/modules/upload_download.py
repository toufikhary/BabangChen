...
""" scape module which contains everything related to \
...
from scape import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY
from scape.utils import progress, humanbytes
from scape.events import register


@register(pattern=r".download(?: |$)(.*)", outgoing=True)
async def download(target_file):
    """ For .download command, download files to the scape's server. """
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the scape's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("scape.session", "config.env"):
