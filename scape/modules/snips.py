...
""" scape module containing commands for keeping global notes. """

from scape.events import register
from scape import CMD_HELP, BOTLOG_CHATID
...
async def on_snip(event):
    """ Snips logic. """
    try:
        from scape.modules.sql_helper.snips_sql import get_snip
...
async def on_snip_save(event):
    ...
    try:
        from scape.modules.sql_helper.snips_sql import add_snip
...
async def on_snip_list(event):
    ...
    try:
        from scape.modules.sql_helper.snips_sql import get_snips
    except AttributeError:
        await event.edit("`Running on Non-SQL mode!`")
        return
...
async def on_snip_delete(event):
    ...
    try:
        from scape.modules.sql_helper.snips_sql import remove_snip
    except AttributeError:
        await event.edit("`Running on Non-SQL mode!`")
        return
